 ...
 * @since 2018-12-28 16:32
 */
class JwtAuthoritiesAuthenticationConverter internal constructor(private val delegate: JwtAuthenticationConverter) : Converter<Jwt, Mono<AbstractAuthenticationToken>> {
    ...
    private fun attrs(jwt: Jwt): Collection<String> {
        ...
        if (scopes is String) {
            return if (StringUtils.hasText(scopes)) {
                Arrays.asList(*scopes.split(" ".toRegex()).dropLastWhile { it.isEmpty() }.toTypedArray())
            } else {
                emptyList()
            }
